...
var BtwPiAiAdapter = class extends PiAiAdapter {
	...
	async *stream(options) {
		...
		if (typeof internal$1.current !== "function") throw new Error("dsh-btw: incompatible PiAiAdapter internals (expected rc.6 current())");
...
function btwPiAiAdapter(runtime, provider, onBoundaryMoved) {
	...
	if (!(candidate instanceof PiAiAdapter || candidate.constructor?.name === "PiAiAdapter") || internal$1.config === void 0) return void 0;
	if (typeof internal$1.config.profiles !== "function" || typeof internal$1.config.resolveApiKey !== "function") return void 0;
...
function installBtwService(ctx, config = {}) {
	...
	if (connection === void 0) throw new Error("dsh-btw requires the Host client-connection service");
	ctx.effect(() => connection.rpc.handle(BTW_RPC_CHANNEL, async (endpoint, payload, transportSignal) => {
